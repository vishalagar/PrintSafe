...
export async function POST(req: NextRequest) {
  ...
  if (expiredDocs) {
    for (const doc of expiredDocs as Pick<DocumentRow, 'id' | 'token' | 'storage_key'>[]) {
      try {
  ...
  if (viewedDocs) {
    for (const doc of viewedDocs as Pick<DocumentRow, 'id' | 'token' | 'storage_key' | 'viewed_at' | 'ttl_after_view'>[]) {
      if (!doc.viewed_at) continue
      const deadline = new Date(doc.viewed_at).getTime() + doc.ttl_after_view * 1000
