...
export async function GET(req: NextRequest, context: RouteContext) {
  ...
  // 2. Unconfirmed upload — the client hasn't confirmed the ciphertext PUT
  // landed in R2 yet (or abandoned it). Treat exactly like a nonexistent
  // document so a token doesn't leak metadata for a blob that may not exist.
  if (doc.status === "pending" && !doc.confirmed_at) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }

  // 3. Status gate — includes 'viewed' to enforce one-time access.
  ...
  // 4. Mark as viewed on first access
  ...
  // 5. Return metadata — ciphertext is proxied via /api/file/[token] (avoids CORS)
  return NextResponse.json({
    iv: doc.iv,
