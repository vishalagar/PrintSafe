...
3. `vercel.json` configures a daily cron job (2 AM UTC) for `/api/cron/cleanup`. Vercel automatically includes `Authorization: Bearer <CRON_SECRET>` in cron requests when the env var is set.
